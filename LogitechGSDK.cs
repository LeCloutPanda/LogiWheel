...
public class LogitechGSDK
{
    ...
    {
        IntPtr ptr = LogiGetStateENGINES(index);
        if (ptr == IntPtr.Zero)
        {
            Debug.Write("Pointer is null");
            return new DIJOYSTATE2ENGINES();
        }

        try
        {
            return Marshal.PtrToStructure<DIJOYSTATE2ENGINES>(ptr);
        }
        catch (System.Exception ex)
        {
            Debug.Write("Error during marshalling: " + ex.Message);
            return new DIJOYSTATE2ENGINES();
        }
    }
